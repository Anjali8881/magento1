<?php 
class Ccc_Salesman_Model_Resource_Attribute extends Mage_Eav_Model_Resource_Entity_Attribute{
	...
	protected function _clearUselessAttributeValues(Mage_Core_Model_Abstract $object){
		...
		if($object->isScopeGlobal()
			...
			&& Ccc_Salesman_Model_Resource_Eav_Attribute::SCOPE_GLOBAL != $origData['is_global']
	){
	...
	public function deleteEntity(Mage_Core_Model_Abstract $object){
		...
		$select = $this->_getReadAdapter()->select()
		          ...
		          ->where('entity_attrinute_id = ?',(int) $object->getEntityAttributeId());
		$result = $this->_getReadAdapter()->fetchrow($select);

		if($result){
			$attribute = Mage::getSingleton('eav/config')
			          ->getAttribute(Ccc_Salesmanattribute_Model_Resource_Salesmanattribute::ENTITY,$result['attribute']);
