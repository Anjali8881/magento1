...
class Ccc_Salesman_Adminhtml_Salesman_SetController extends Mage_Adminhtml_Controller_Action
{

	protected function _setTypeId()
    {
        Mage::register('entityType',
            Mage::getModel('salesman/salesman')->getResource()->getTypeId());
    }
	...
	public function indexAction()
	{
        ...
        $this->_addBreadcrumb(
            Mage::helper('salesman')->__('Manage salesman Sets'),
            Mage::helper('salesman')->__('Manage salesman Sets'));

        $this->_addContent($this->getLayout()->createBlock('salesman/adminhtml_salesman_attribute_set_toolbar_main'));
      
        $this->_addContent($this->getLayout()->createBlock('salesman/adminhtml_salesman_attribute_set_grid'));
	...
	public function addAction()
    {
        $this->_title($this->__('salesman'))
             ->_title($this->__('Attributes'))
             ->_title($this->__('Manage salesman Attribute Sets'))
        ...
        $this->_addContent($this->getLayout()->createBlock('salesman/adminhtml_salesman_attribute_set_toolbar_add'));
    ...
    public function editAction()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('salesman')->__('salesman'), Mage::helper('salesman')->__('salesman'));
        $this->_addBreadcrumb(
            Mage::helper('salesman')->__('Manage salesman Attribute Sets'),
            Mage::helper('salesman')->__('Manage salesman Attribute Sets'));

        $this->_addContent($this->getLayout()->createBlock('salesman/adminhtml_salesman_attribute_set_main'));
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $model->save();
            $this->_getSession()->addSuccess(Mage::helper('salesman')->__('The attribute set has been saved.'));
        } catch (Mage_Core_Exception $e) {
            $this->_getSession()->addError($e->getMessage());
            $hasError = true;
        } catch (Exception $e) {
            $this->_getSession()->addException($e,
                Mage::helper('salesman')->__('An error occurred while saving the attribute set.'));
            $hasError = true;
        }
