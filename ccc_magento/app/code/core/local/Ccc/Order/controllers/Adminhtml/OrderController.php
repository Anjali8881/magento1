...
class Ccc_Order_Adminhtml_OrderController extends Mage_Adminhtml_Controller_Action {
	...
	public function customerGridAction() {
		$this->loadLayout();
		$this->_setActiveMenu('order');
		$this->_title('Customer Grid');

		$this->_addContent($this->getLayout()->createBlock('order/adminhtml_order_customer'));
	...
	protected function getCart() {
		...
		$cart->setCreatedDate(date('dd/mm/yy  h:i:s'));
		$cart->save();
		return $cart;
	}
		...
		if ($products) {
			foreach ($products as $key => $id) {
				...
				if (in_array($id, $itemId)) {
					...
					date_default_timezone_set('Asia/Kolkata');
					$cartItem->setCreatedDate(date('dd/mm/yy  h:i:s'));
				}
				$cartItem->save();
			}
		}
	...
	public function shippingAddressAction() {
		...
		if (array_key_exists('same_as_billing', $address)) {
			...
			if ($cartAddress = $cart->getCartShippingAddress()) {
				...
				$model->setAddressType('shipping');
				date_default_timezone_set('Asia/Kolkata');
				$model->setCreatedData(date('dd/mm/yy  h:i:s'));
	...
	public function placeOrderAction() {
		...
		if ($cartItems->count() <= 0) {
			Mage::getSingleton('adminhtml/session')->addError('One Item Is Compulsory To Add');
			$this->_redirect('*/adminhtml_order/grid', array('_current' => true));
			return;
		}
		if (!$billingAddress) {
		...
		date_default_timezone_set('Asia/Kolkata');
		$orderModel->setCreatedDate(date('dd/mm/yy  h:i:s'));
		$orderModel->save();
		...
		$orderAddress->setCreatedDate(date('dd/mm/yy  h:i:s'));
		$orderAddress->save();
